import { cn } from "@/lib/utils";
import { IntentLink } from "@/components/site/intent-link";
import { Frame, PillLink } from "@/components/site/product/primitives";
import { HomeHeading } from "@/components/site/home/heading";
import { TYPE, WEIGHT } from "@/components/site/home/type";
import { faqJsonLd, type FaqData, type FaqItem } from "@/lib/pages/custom-saas-platforms";
import "@/components/site/home/doubts.css";

/* ------------------------------------------------------------------ *
 * #faq — the doubts that remain before a build: whether we only build
 * for voice, what "complete" means, how complex, what it costs, whether
 * it needs AI, who owns the code, what the grants are, and where the
 * data lives. One native <details> row a question (ten on the SaaS page,
 * eleven on the Automations page), all closed.
 *
 * THE LANDING'S #faq, COPIED, NOT IMPORTED. The markup is doubts.tsx's,
 * row for row, with this page's words, and it wears the landing's own
 * classes (`home-faq`, `home-doubts-*`, from doubts.css and home.css), so
 * the two read as one design and doubts.tsx stays untouched: it renders
 * the homepage's constant, and a prop for ours would be a refactor of a
 * landing component for one page.
 *
 * A server component: nothing here runs in the browser but the
 * heading's reveal and the links' prefetch on intent. The answers are in
 * the server's HTML, so Ctrl+F finds them and opens their row; the
 * height eases open in CSS (home.css `.home-faq`, `::details-content`
 * with `interpolate-size`), the answer settles in through
 * `@starting-style` (doubts.css), and every other state is a selector on
 * [open], :hover, :active or :focus-visible.
 *
 * At lg the heading column stays in view while the rows beside it open,
 * so a long answer never leaves the reader without the section's
 * question; below lg it is one column, heading first. The heading's key
 * is the page's one quiet phrase (`keyTone: "quiet"`, muted, not violet):
 * these are questions, not a payoff.
 *
 * The ordinals number the rows for the eye only; they are hidden from
 * assistive tech, which gets the h3 questions and no numbering noise.
 *
 * Two answers end somewhere the reader can go (the phone-agent page, the
 * privacy policy). A route goes through IntentLink, which prefetches on
 * a pointer, a touch or focus rather than on sight; a fragment or a
 * `tel:` would be a plain anchor, the browser's own jump.
 *
 * The FAQPage structured data is built by the data module's
 * `faqJsonLd` from the very rows drawn here, never typed out:
 * structured data that disagrees with the page is worse than none.
 * ------------------------------------------------------------------ */

/** The one link an answer may end on, in the landing's `home-doubts-where` markup. */
const WHERE =
  "home-doubts-where group/where mt-1 -mb-2.5 inline-flex min-h-11 items-center gap-1.5 rounded-full text-[15px] leading-[22px] text-pp-ink transition-colors duration-200 hover:text-(--home-plum) focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-pp-ink";

function Where({ where }: { where: NonNullable<FaqItem["where"]> }) {
  const words = (
    <>
      {/* The underline on the words only: on the flex parent it would
          carry into the arrow and break across the gap. */}
      <span className="underline decoration-pp-ink/25 decoration-1 underline-offset-[5px] transition-[text-decoration-color] duration-200 group-hover/where:decoration-current">
        {where.label}
      </span>
      <span aria-hidden className="home-doubts-arrow">
        →
      </span>
    </>
  );
  return where.href.startsWith("/") ? (
    <IntentLink href={where.href} className={WHERE}>
      {words}
    </IntentLink>
  ) : (
    <a href={where.href} className={WHERE}>
      {words}
    </a>
  );
}

export function Faq({ data }: { data: FaqData }) {
  return (
    <section id="faq" aria-labelledby="faq-title" className="scroll-mt-8">
      <Frame className="grid gap-10 lg:grid-cols-[384px_minmax(0,1fr)] lg:gap-12">
        <div className="lg:sticky lg:top-28 lg:self-start">
          <HomeHeading
            id="faq-title"
            eyebrow={data.eyebrow}
            title={data.title}
            titleKey={data.key}
            keyTone={data.keyTone}
          />
          <div className="mt-6 flex flex-wrap items-center gap-x-4 gap-y-3">
            <PillLink
              href={data.talk.href}
              variant="secondary"
              size="sm"
              // 36px pill, 44px target: the hit area reaches 4px past
              // each edge, clear of anything else that takes a tap.
              className="relative after:absolute after:inset-x-0 after:-inset-y-1"
            >
              {data.talk.label}
            </PillLink>
            <p className={cn(TYPE.meta, "tabular-nums")} translate="no">
              {data.phone}
            </p>
          </div>
        </div>

        <div className="home-faq border-t border-pp-rule">
          {data.items.map((item, i) => (
            <details key={item.id} className="home-doubts-row border-b border-pp-rule pb-[15px]">
              <summary className="home-doubts-q flex items-baseline pt-[26px] pb-[10px]">
                <span aria-hidden className={cn(TYPE.mono, "home-doubts-n w-8 shrink-0 text-pp-muted md:w-14")}>
                  {String(i + 1).padStart(2, "0")}
                </span>
                <h3 className={cn(TYPE.h3, "min-w-0 flex-1 text-pretty")} style={{ fontWeight: WEIGHT.h3 }}>
                  {item.q}
                </h3>
                {/* 32px disc on a 28px line: -2px above and below centres it
                    on the first line and keeps it out of the row's height. */}
                <span
                  aria-hidden
                  className="home-doubts-disc -my-0.5 ml-4 grid size-8 shrink-0 place-items-center self-start rounded-full md:ml-6"
                >
                  <svg viewBox="0 0 12 12" fill="none" className="size-3">
                    <path d="M6 1.25v9.5M1.25 6h9.5" stroke="currentColor" strokeWidth={1.5} strokeLinecap="round" />
                  </svg>
                </span>
              </summary>

              <div className="home-doubts-a pb-[10px] pl-8 md:pl-14">
                {/* 600, not the 640 ceiling: a line of Inter 16 at 600
                    holds ~78 characters, the comfortable measure. */}
                <p className={cn(TYPE.lead, "max-w-[600px] text-pretty")}>{item.a}</p>
                {/* 22px of text in a 44px target; the -10px bottom margin
                    hands the target's lower half back to the row's padding,
                    so a row that ends on a link keeps the same 26px foot. */}
                {item.where ? <Where where={item.where} /> : null}
              </div>
            </details>
          ))}
        </div>
      </Frame>

      <script
        type="application/ld+json"
        // Our own constants, escaped anyway: a `<` in an answer would
        // otherwise be free to close this element early.
        dangerouslySetInnerHTML={{ __html: JSON.stringify(faqJsonLd(data.items)).replace(/</g, "\\u003c") }}
      />
    </section>
  );
}
